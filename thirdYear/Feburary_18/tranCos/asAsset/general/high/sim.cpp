...
const double sim :: TF_I = 1.0/2*3;
const double sim :: TF_II = 2.0/2*3;
const double sim :: TP_I = 0.1/2*3;
const double sim :: TP_II = 0.2/2*3;
const double sim :: ShorPen_Rate = 2.0;
...
void sim :: simulation () {
	...
	double temp_income = ReturnRate * *assetLevel;
	if(cash_action < 0){
	...
	*cashLevel += cash_flow;
	...
	//
	currentReward = cash_flow - cost_by_trans - cost_by_shortage + temp_income;
}
