...
int main () {
	...
	const double TF_I = 1.0/2*3;
	const double TF_II = 2.0/2*3;
	const double TP_I = 0.1/2*3;
	const double TP_II = 0.2/2*3;
	...
	for(int i = 0; i <= maxC; ++i){
		...
		for(int j = 0; j <= maxA; ++j){
			...
			if(Action < 0.0001 && Action > -0.0001){
				...
				a_move = State_to_Value(j,DISC) - Action - TransactionCost + returnRate * State_to_Value(j,DISC);
				C_justMove = Action ;
				A_justMove = - Action - TransactionCost + returnRate * State_to_Value(j,DISC);
				grey = 1;
				...
				a_move = State_to_Value(j,DISC) - Action+ returnRate * State_to_Value(j,DISC);
				C_justMove = Action - TransactionCost ;
				A_justMove = - Action + returnRate * State_to_Value(j,DISC);
				grey = 1;
				cmove << c_move << endl;
				amove << a_move << endl;
			}
