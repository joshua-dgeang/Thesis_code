	...
	for(int i = 0; i <= maxC; ++i){
		...
		for(int j = 0; j <= maxA; ++j){
			...
			if(Action < 0.0001 && Action > -0.0001){
				...
				c_move = State_to_Value(i,DISC) + Action + returnRate * State_to_Value(j,DISC);
				...
				c_move = State_to_Value(i,DISC) + Action - TransactionCost + returnRate * State_to_Value(j,DISC);
