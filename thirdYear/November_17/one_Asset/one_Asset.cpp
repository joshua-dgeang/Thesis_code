...
void one_Asset :: OutPut() {
	valueoutput.open("one_asset_value.csv");
	policyoutput.open("one_asset_policy.csv");
	if(!valueoutput.is_open()){
		cerr << "Error with value output!"<<endl;
		exit(EXIT_FAILURE);
	}
	if(!policyoutput.is_open()){
		cerr << "Error with policy output!"<<endl;
		exit(EXIT_FAILURE);
	}
	else;
	for(int i = 0; i <= MaxC; ++i){
