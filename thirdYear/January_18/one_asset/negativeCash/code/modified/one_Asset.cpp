...
const double one_Asset :: SCR = 1.0;
...
one_Asset :: one_Asset (double tf1, double tf2, double tp1, double tp2):TF_I(tf1), TF_II(tf2), TP_I(tp1), TP_II(tp2),c_mean(0) {
	...
	for( int i = 0; i <= MaxC; ++i){
		...
		OptAction_index[i] = 0;
		double min, max;
		min = (State_to_Value(i)-TF_I-State_to_Value(0))/(1+TP_I);
		if(min < 0)
			min = 0;
...
double one_Asset :: State_to_Value(int state){
	return (state-100)*0.2;
}
int one_Asset :: Value_to_State(double value){
	int s = floor (value*5+0.5);
	s = s+100;
	if(s > MaxC)
		s = MaxC;
	else if (s < 0)
...
double one_Asset :: interpolation(double x){
	int sf = floor(x*5)+100;
	int sc = ceil(x*5)+100;
	if (sc > MaxC){
		sc = MaxC;
		sf = MaxC;
	}
		//x = State_to_Value(MaxC);
	else if (sf < 0){
		sf = 0;
		sc = 0;
	}
	...
	//xf = floor (x*5);
	//xc = ceil (x*5);
		return (x*5+100-sf) * (OldV[sc] - OldV[sf]) + OldV[sf];
}
