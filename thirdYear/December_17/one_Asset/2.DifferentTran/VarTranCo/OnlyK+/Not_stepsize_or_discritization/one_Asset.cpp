...
//const double one_Asset :: TF_I = 1.0;//K-****
//const double one_Asset :: TF_II = 2.4;//K+
//const double one_Asset :: TP_I = 0.1;//k-****
//const double one_Asset :: TP_II = 0.24;//k+
...
const double one_Asset :: Action_size = 0.05;
const int one_Asset :: MaxC = 400;
one_Asset :: one_Asset (double tf1, double tf2, double tp1, double tp2):TF_I(tf1), TF_II(tf2), TP_I(tp1), TP_II(tp2),c_mean(0) {
...
double one_Asset :: interpolation(double x){
	...
	xf = floor (x*10);
	xc = ceil (x*10);
		return (x*10 - xf) * (OldV[xc] - OldV[xf]) + OldV[xf];
}
