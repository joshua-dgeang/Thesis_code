...
DP :: DP () : maxC(1000), maxA(3000), smallNum(0.0001), c_mean(0.0), c_std(2.5), discR(1.0/1.02), action_step_size(1.0){
...
double DP :: interpolation (double x, double y) {
	if(x * 10.0 > maxC) x = maxC * 0.1;
	if(y * 10.0 > maxA) y = maxA * 0.1;
	if(x < 0) x = 0;
	if(y < 0) y = 0;
	xf = floor(x*10.0);
	yf = floor(y*10.0);
	...
	VC = OldV[xc][yc];
	VD = OldV[xf][yc];
	VF = (x*10.0-xf) * (VB - VA) + VA;
	VE = (x*10.0-xf) * (VC - VD) + VD;
	VG = (y*10.0-yf) * (VE -VF) + VF;
	return VG;
}
