...
#include <cmath>
using namespace std;
const int simu :: MaxC = 100;
const int simu :: MaxA = 300;
...
simu :: simu (int * const S_x, int * const S_y, bool* const stop, double d, int t): S_c(S_x),S_a(S_y),stp(stop),cash_demand(d),cash_transfer(t){
	...
	simulation();
	//make sure cash_simu and asset_simu do not exceed their limits; only needed in dynamic programming;
...
void simu :: simulation () {
	...
	asset_simu = *S_a * 1.0;
		if(cash_transfer < 0){
			cost_by_trans = TF_I + TP_I * abs(cash_transfer);
			cash_simu = cash_simu - abs(cash_transfer) - cost_by_trans;
			asset_simu += abs(cash_transfer);
		}
		else if (cash_transfer > 0){
			cost_by_trans = TF_II + TP_II * abs(cash_transfer);
			asset_simu = asset_simu - cost_by_trans - abs(cash_transfer);
			cash_simu += abs(cash_transfer);
		}
		else
			cost_by_trans = 0;
	...
	else if (cash_simu >= MaxC && asset_simu >= MaxA){
		*stp = 1;
		*S_c = MaxC;
		*S_a = MaxA;
	}
