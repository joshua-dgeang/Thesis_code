...
one_Asset :: one_Asset (double m, double tf1, double tf2, double tp1, double tp2):TF_I(tf1), TF_II(tf2), TP_I(tp1), TP_II(tp2),c_mean(m) {
...
double one_Asset :: interpolation(double x){
	...
	//xf = floor (x*5);
	//xc = ceil (x*5);
		return (x+100-sf) * (OldV[sc] - OldV[sf]) + OldV[sf];
}
